...
export function TripProvider({ children }) {
  ...
  try {
    const saved = localStorage.getItem('runningstars_trips');
  ...
  const [darkMode, setDarkMode] = useState(() => {
    return localStorage.getItem('runningstars_dark') === 'true';
  });

  // Persist trips
  useEffect(() => {
    localStorage.setItem('runningstars_trips', JSON.stringify(trips));
  }, [trips]);

  // Persist & apply dark mode
  useEffect(() => {
    localStorage.setItem('runningstars_dark', darkMode);
